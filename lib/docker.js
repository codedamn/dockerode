...
Docker.prototype.createContainer = function(opts, callback) {
	...
	if (callback === undefined) {
		return new this.modem.Promise(function(resolve, reject) {
			self.modem.dial(opts && opts.cookie, opts && opts.serverid, optsf, (err, data) => {
				if (err) {
					return reject(err)
				}
				resolve(self.getContainer(data.Id, opts && opts.cookie, opts && opts.serverid))
			})
		})
	} else {
		this.modem.dial(opts && opts.cookie, opts && opts.serverid, optsf, function(err, data) {
			if (err) return callback(err, data)
			callback(err, self.getContainer(data.Id, opts && opts.cookie, opts && opts.serverid))
		})
	}
}

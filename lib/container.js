 ...
 */
var Container = function(modem, id, cookie, serverid) {
	this.modem = modem
	this.id = id
	this.cookie = cookie
...
Container.prototype.inspect = function(opts, callback) {
	...
	if (args.callback === undefined) {
		return new this.modem.Promise(function(resolve, reject) {
			self.modem.dial(self.cookie, self.serverid, optsf, function(err, data) {
				if (err) {
					return reject(err)
				}
				resolve(data)
		...
		this.modem.dial(self.cookie, self.serverid, optsf, function(err, data) {
...
Container.prototype.exec = function(opts, callback) {
	...
	if (args.callback === undefined) {
		return new this.modem.Promise(function(resolve, reject) {
			self.modem.dial(self.cookie, self.serverid, optsf, (err, data) => {
				if (err) {
					return reject(err)
				}
				resolve(new Exec(self.modem, data.Id, self.cookie, self.serverid))
			})
		})
	} else {
		this.modem.dial(self.cookie, self.serverid, optsf, (err, data) => {
			if (err) return args.callback(err, data)
			args.callback(err, new Exec(self.modem, data.Id, self.cookie, self.serverid))
		})
	}
}
...
Container.prototype.attach = function(opts, callback) {
	...
	if (args.callback === undefined) {
		return new this.modem.Promise(function(resolve, reject) {
			self.modem.dial(self.cookie, self.serverid, optsf, function(err, stream) {
				if (err) {
					return reject(err)
				}
				resolve(stream)
		...
		this.modem.dial(self.cookie, self.serverid, optsf, function(err, stream) {
